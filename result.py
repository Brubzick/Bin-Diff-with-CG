def GetResult(features1, features2, simMatrix, matching):
    ...
    print(row)
    result.append(row)
    for site in matching: # only full match is printed, but secondary match is also used for final score calcaulation.
        if site[2] == 1:
            row = [features1[site[0]]['name'], features1[site[0]]['size'], features2[site[1]]['name'], features2[site[1]]['size'], simMatrix[site[0],site[1]]]
            print(row)
            result.append(row)
